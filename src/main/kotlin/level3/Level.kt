...
fun solvePerLine(input: String): String {
    ...
    var history = mutableListOf(current)
    ...
    for ((index, value) in history.withIndex()) {
        area += (history[j].x + value.x) * (history[j].y - value.y)
        j = index
    }
    area = abs(area/2.0).roundToInt()
    return count(actions).toString() + " " + (abs(max[2] - max[0])).times(abs(max[3] - max[1])).toString() + " " + area
}
